...
def get_items_by_partial_name(item_names):
    ...
    cleaned_names = [name for name in item_names]
    items = []
...
@frappe.whitelist()
def get_repair_floor_stock():
    """Return item stock for the Repair Floor - DTPL warehouse only.
    Runs with ignore_permissions since Bin access shouldn't be granted broadly."""
    ...
    # Aggregate qty per item_code in case of duplicates
    stock_map = {}
    for b in bins:
    ...
    items = frappe.get_all(
        "Item",
        filters={"name": ["in", list(stock_map.keys())]},
        fields=["name", "item_name"],
        ignore_permissions=True
    )
    item_name_map = {i.name: i.item_name or i.name for i in items}

    result = [
        {"item_code": code, "item_name": item_name_map.get(code, code), "stock": qty}
